...
(function (SBA) {
  ...
  function renderOrderSummary(items) {
    ...
    listEl.innerHTML = items.map(function (it) {
      var displayName = SBA.i18n.pick(it.name);
      // For weight-based items show "Name — 250 g" instead of "×1"
      var qtyLabel = it.soldByWeight
        ? SBA.i18n.t("weight.cartWeightLabel", { w: it.weightG })
        : it.qty + "×";
      return (
        '<div class="order-summary-item">' +
          '<span class="qty">' + SBA.escapeHTML(qtyLabel) + "</span>" +
          '<span class="name">' + SBA.escapeHTML(displayName) + "</span>" +
    ...
    form.addEventListener("submit", function (e) {
      ...
      var payload = {
        ...
        products: currentItems.map(function (it) {
          // For weight-based items, the name sent to the backend (Google Sheets /
          // Telegram) includes the weight so staff can read it at a glance.
          var frName = SBA.i18n.pick(it.name, "fr");
          if (it.soldByWeight) {
            frName = frName + " (" + it.weightG + " g)";
          }
          return {
            id:          it.soldByWeight ? it.productId || it.id : it.id,
            name:        frName,
            qty:         it.qty,
            price:       it.price,
            subtotal:    it.qty * it.price,
            soldByWeight: it.soldByWeight || false,
            weightG:     it.soldByWeight ? it.weightG : null,
            pricePerKg:  it.soldByWeight ? it.pricePerKg : null
          };
        }),
        total: total
      };
