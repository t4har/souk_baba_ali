...
(function (SBA) {
  ...
  SBA.loadCatalog = function () {
    ...
    return Promise.all([
      ...
      // Only display products that have a real product image.
      // Products without an image remain in products.json but are hidden
      // from the website until their image is added.

      SBA.catalog.products = results[0].filter(function (product) {
  ...
  function stockBadge(stock) {
    if (stock <= 0)
      return (
        ...
        SBA.i18n.t("products.outOfStock") +
        "</span>"
      );
    if (stock <= 5)
      return (
        '<span class="stock-badge low">' +
        SBA.i18n.t("products.lowStock", { n: stock }) +
        "</span>"
      );
    return (
      '<span class="stock-badge">' + SBA.i18n.t("products.inStock") + "</span>"
    );
  }
  ...
  function cardMedia(p, index, name, badge) {
    return (
      '<article class="product-card" style="animation-delay:' +
      ...
      '"' +
      (p.soldByWeight
        ? ' data-sold-by-weight="1" data-price-per-kg="' + p.pricePerKg + '"'
        : "") +
      ">" +
      '<div class="product-media">' +
      badge +
      '<img src="' +
      SBA.escapeHTML(p.image) +
      '" alt="' +
      SBA.escapeHTML(name) +
  ...
  function unitCardHTML(
    ...
    return (
      ...
      SBA.escapeHTML(categoryLabel) +
      "</span>" +
      '<h3 class="product-name">' +
      SBA.escapeHTML(name) +
      "</h3>" +
      '<p class="product-desc">' +
      SBA.escapeHTML(description) +
      "</p>" +
      '<div class="product-footer">' +
      '<div class="product-price">' +
      SBA.formatPrice(p.price) +
      "</div>" +
      "</div>" +
      '<div class="product-row-actions">' +
      '<div class="qty-stepper" role="group">' +
      '<button type="button" class="qty-dec" aria-label="-" ' +
      (disabled ? "disabled" : "") +
      ">−</button>" +
      '<input type="text" class="qty-value" value="1" inputmode="numeric" readonly>' +
      '<button type="button" class="qty-inc" aria-label="+" ' +
      (disabled ? "disabled" : "") +
      ">+</button>" +
      "</div>" +
      '<button type="button" class="add-cart-btn" aria-label="' +
      SBA.i18n.t("products.addAria") +
      '" ' +
      (disabled ? "disabled" : "") +
      ">" +
  ...
  function weightCardHTML(
    ...
    var defaultPrice = SBA.formatPrice(
      calcWeightPrice(p.pricePerKg, DEFAULT_WEIGHT),
    );
    ...
    return (
      ...
      '<small class="per-kg-label">' +
      SBA.escapeHTML(perKgLabel) +
      "</small>" +
      "</div>" +
      /* Weight selector */
      ...
      '<input type="number" class="weight-custom-input" placeholder="' +
      SBA.escapeHTML(customPlaceholder) +
      '" min="50" step="50" aria-label="' +
      SBA.escapeHTML(customPlaceholder) +
      '">' +
      '<span class="weight-price-preview" aria-live="polite">' +
      defaultPrice +
      "</span>" +
      "</div>" +
      '<span class="weight-error"></span>' +
      "</div>" +
      '<button type="button" class="add-cart-btn-weight" ' +
      (disabled ? "disabled" : "") +
      ">" +
  ...
  SBA.productCardHTML = function (p, index) {
    ...
    if (p.bestSeller)
      badge =
        ...
        SBA.i18n.t("products.badgeBest") +
        "</span>";
    else if (p.featured)
      badge =
        '<span class="product-badge">' +
        SBA.i18n.t("products.badgeFeatured") +
        "</span>";
  ...
  function getSelectedWeight(card) {
    var activeBtn = card.querySelector(
      ".weight-quick-btn[aria-pressed='true']",
    );
  ...
  function updateWeightPreview(card, pricePerKg) {
    ...
    if (!preview) return;
    if (weightG) {
      preview.textContent = SBA.formatPrice(
        calcWeightPrice(pricePerKg, weightG),
      );
    }
  }
  ...
  SBA.getCategoryLabel = function (slug) {
    var cat = (SBA.catalog.categories || []).find(function (c) {
      return c.slug === slug;
    });
    ...
    SBA.loadCatalog()
      ...
      .catch(function (err) {
        ...
        [featuredEl, bestEl].forEach(function (el) {
          if (el)
            el.innerHTML =
              '<p class="text-muted">Impossible de charger les produits pour le moment.</p>';
        });
      });
  }
