...
(function (SBA) {
  ...
  function cartItemTemplate(item) {
    ...
    var removeSvg = '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M3 6h18M8 6V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2m3 0-1 14a2 2 0 0 1-2 2H8a2 2 0 0 1-2-2L5 6"/></svg>';

    if (item.soldByWeight) {
      // Weight-based display: "Name  [250 g]  qty-stepper  subtotal"
      // unit price line shows "250 g · 2 200 DA / kg"
      var weightLabel = SBA.i18n.t("weight.cartWeightLabel", { w: item.weightG });
      var unitLine = SBA.i18n.t("weight.cartUnitLine", {
        w:     item.weightG,
        price: SBA.formatPrice(item.pricePerKg)
      });
      return (
        '<li class="cart-item" data-id="' + SBA.escapeHTML(item.id) + '">' +
          ...
          '<div class="cart-item-info">' +
            ...
            '<span class="weight-badge">' + SBA.escapeHTML(weightLabel) + "</span>" +
            '<div class="unit-price">' + SBA.escapeHTML(unitLine) + "</div>" +
          ...
          '<div class="cart-item-controls">' +
            ...
            '<button type="button" class="cart-item-remove">' + removeSvg + SBA.i18n.t("cart.remove") + "</button>" +
          "</div>" +
        "</li>"
      );
    }

    // Unit-based display (unchanged)
    return (
      '<li class="cart-item" data-id="' + SBA.escapeHTML(item.id) + '">' +
        '<img src="' + SBA.escapeHTML(item.image) + '" alt="' + SBA.escapeHTML(name) + '" loading="lazy" width="84" height="84">' +
        '<div class="cart-item-info">' +
